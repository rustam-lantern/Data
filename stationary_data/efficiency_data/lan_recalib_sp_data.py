...
def Read_Sp_Data(infilename):
    ...
    for num, line in enumerate(f, 1):
        ...
        columns = line.split()
        lbins += [int(columns[0])]
        lcounts += [int(columns[1])]
    #print header
    a0=float(header.split()[5])
    a1=float(header.split()[6])
    a2=float(header.split()[7])
    energy = [return_energy(lbins[i],a0,a1,a2) for i in range(len(lbins))]
    print "Read data file:",infilename," entries:",num," ",a0," ",a1," ",a2
    return energy,lcounts
...
def Save_Spe_Data(header,ebins,counts):
    outfile=OUTDIR+"/"+filename_data+".spe"
    ofilename  = open(outfile,'w')
    print >>ofilename, "%s"%(header),
    for i in range(0,len(ebins)):
        print >>ofilename,"%4d %d"%(ebins[i],counts[i])
...
print ("Usage: %s filename.sp" %str(sys.argv[0]) )
...
[ebins,counts]=Read_Sp_Data(filename_data)
Save_Spe_Data(header,ebins,counts)
