...
def time_data_old(sampling_time,binid):
    ...
    #Calculate total count rates in the bins
 #   bincounts = []
 #   testcounts = []
 #   bincounts_time= [[0]*(len(energyBins))]*nsamples#2D group indexing
 #   for binid in range(len(energyBins)):
 ...
 #       tmpcounts =[]
 #       for isample in range(nsamples):
 #           tmpcounts += [(sum(1 for i in range(len(time)) if (time[i] >= time[index_range[isample][0]]) & (time[i] <= time[index_range[isample][1]]) &  (channel[i] > energyBins[binid][0]) &   (channel[i] <= energyBins[binid][1]) ))]
 #       bincounts_time[binid] = tmpcounts
...
[time,bdt]=time_dataC(sampling_time,binid)

print "sum of counts:", "%6.3f"%float(sum(bdt)/len(bdt)),bd[binid]
#for i in range(0,len(bdt)):
#    print i,"%6.1f"%(time[i]),"%6.1f"%(bdt[i])

PlotTimeData(time,bdt,sampling_time,binid)
PlotBinData(bd)
